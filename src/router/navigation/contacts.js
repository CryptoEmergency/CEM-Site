import {jsx,jsxFrag,setVariable,setAction,makeDOM,getStorage} from '@betarost/cemjs'
...
const make = function(){
    ...
    return (
        <div class="contacts_container">
            cool
        {/* <img class="background_vector" src="/assets/image/background/contacts_vector.svg"/>
        <div class="contacts_content">
            <div class="contacts_form_block">
                <div class="contacts_form">
                    <h4>{{lang.h.contact}}</h4>
                    <p>{{lang.p.writeUs}}</p>
                    <form id="contactsForm" onsubmit="sendFormWithoutReload(this, event)" data-button_id="sendContacts">
                        ...
                        <div>
                            ...
                            <div class="contacts_form_name_icon">
                                <input id="contacts_name"  placeholder="{{lang.placeholder.name}}" {{#if myInfo.nickname}} value="{{myInfo.nickname}}" readonly{{/if}} class="contacts_form_name" type="text">
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="contacts_form_email_icon">
                                <input id="contacts_email" placeholder="{{lang.placeholder.email}}" {{#if myInfo.email}} value="{{myInfo.email}}" readonly{{/if}} class="contacts_form_email" type="text">
                            </div>
                        </div>
                        <div>
                            <label for="">{{lang.label.massage}}</label>
                            <div class="error-div">{{lang.error_div.not_empty_input}}</div>
                            <div>
                                <textarea id="contacts_text" placeholder="{{lang.placeholder.message}}"></textarea>
                            </div>
                            <div id="sendContacts" data-action="sendContacts">
                                <a class="btn-contacts" ><span>{{lang.button.send}}</span></a>
                            </div>
                        </div>
                    </form>
                </div>
