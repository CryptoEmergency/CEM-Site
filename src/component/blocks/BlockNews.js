...
const BlockNews = async function ({ Static }) {
  await initOne(async () => {
    Static.apiFilter = makeFilter(Static);
    if (Static.type != "media") {
      await fn.restApi.getCategories({
        cache: true,
        name: Static.nameRecords + "Category",
        filter: Static.apiFilter,
      });
    }
    await fn.restApi.getNews({
    ...
    <div class="blog_page">
      ...
      {() => {
        if (Static.type == "media") {
          ...
          return <div class="tags">{arrReturn}</div>;
        } else {
          return (
            <div class="tags">
              <Tags
                ...
                classActive={
                  Static.activeCategory == "All" ? "tag_button_active" : ""
                }
                type="All"
              />
              {() => {
                if (Variable[Static.nameRecords + "Category"]) {
                  let arrReturn = Variable[
                    Static.nameRecords + "Category"
                  ].list_records
              ...
              Variable[Static.nameRecords].list_records.map((item) => {
                return (
                  ...
                  >
                    ...
                    <p class="blog_new_title">
                      {fn.sliceString(item.title, 85)}
                    </p>
                    <span class="blog_new_text">
                      {fn.sliceString(item.preview, 170)}
                    </span>
